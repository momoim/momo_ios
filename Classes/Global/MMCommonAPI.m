...
+ (NSString*)getDetailURL:(NSUInteger)typeId applicationId:(uint64_t)appId {
	...
	return [NSString stringWithFormat:@"%@?&oauth_token=%@&oauth_token_secret=%@&timestamp=%llu&class=%u&id=%llu&web=2&source=2", 
			REQUEST_DETAIL_URL, token, secret,  (long long)time(NULL), typeId, appId];
}
...
+ (NSString*)getIMLongTextURL:(NSString*)msdId {
	...
	return [NSString stringWithFormat:@"%@?&oauth_token=%@&oauth_token_secret=%@&timestamp=%llu&class=im&method=show_text&source=2&id=%@", 
			REQUEST_DETAIL_URL, token, secret, (long long)time(NULL), msdId];
}
...
+ (MMWebViewController*)openUrl:(NSString*)url {
    ...
    if ([url hasPrefix:@"http://"]) {
        ...
        if (navigationController.viewControllers.count > 1) {
            showInView = navigationController.visibleViewController.view;
        } else {
            showInView = tabBarController.view;
        }
        
        NSString* telNumber = [url substringFromIndex:6];
        [UIActionSheet actionSheetWithTitle:telNumber
                                    message:nil
         ...
         if (0 == buttonIndex) {
             ...
             [self sendMessage:telNumber];
         }
         } onCancel:nil];
    } else if ([url hasPrefix:@"momonav://"]) {
        //地图导航
        NSString* tmpString = [url substringFromIndex:10];
        ...
        for (NSString* paraString in tmpArray) {
            if ([paraString hasPrefix:@"latitude"]) {
                latitude = [[paraString substringFromIndex:9] doubleValue];
            } else if ([paraString hasPrefix:@"longitude"]) {
                longitude = [[paraString substringFromIndex:10] doubleValue];
            } else if ([paraString hasPrefix:@"is_correct"]) {
                isCorrect = [[paraString substringFromIndex:11] boolValue];
            }
        }
        
        MMMapViewController* viewController = [[[MMMapViewController alloc] init] autorelease];
        CLLocationCoordinate2D addressCoordinate;
        addressCoordinate.latitude = latitude;
        addressCoordinate.longitude= longitude;
        
        viewController.friendCoordinate = addressCoordinate;
        viewController.shouldGetFriendGPSOffset = !isCorrect;
...
+ (MMMomoUserInfo*)loginUserInfo {
    ...
    friendInfo.realName = [[MMLoginService shareInstance] getLoginRealName];
    friendInfo.uid = [[MMLoginService shareInstance] getLoginUserId];
    friendInfo.avatarImageUrl = [[MMLoginService shareInstance] avatarImageURL];
    return friendInfo;
}
...
+ (void)showAlertHud:(NSString*)text detailText:(NSString*)detailText {
    ...
    hud.yOffset -= 50;
    hud.labelText = text;
    hud.detailsLabelText = detailText ? detailText : @"";
    hud.removeFromSuperViewOnHide = YES;
    [window addSubview:hud];
    [hud show:YES];
    
    [hud performSelector:@selector(hideHub) withObject:nil afterDelay:HUB_AUTO_HIDE_DELAY_TIME];
}
#endif
