...
- (void)displayImage {
	if (index != NSNotFound && photoImageView.image == nil) {
		...
		// Get image
		NSObject* imgObject = [self.photoBrowser imageObjectAtIndex:index];
		if (imgObject) {
			
			// Hide spinner
			[spinner stopAnimating];
			
			if ([imgObject isKindOfClass:[UIImage class]]) {
				photoImageView.image = (UIImage*)imgObject;
			} else {
				//GIF image
				[photoImageView setGIF_frames:(NSMutableArray*)imgObject];
			}
...
- (void)setMaxMinZoomScalesForCurrentBounds {
    ...
    CGFloat minScale = xScale;//MIN(xScale, yScale);                 // use minimum of these to allow the image to become fully visible
    ...
    boundsSize = photoImageView.frame.size;
	[self setNeedsLayout];
